...
export default function ShopManagerEdit({ params }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.put(`update-shop-manager/${UserID}`, {
        Area: area,
        PinCode: pincode,
        RoleID: roleId
      }, {
  ...
  return (
    <div className="container mt-5">
      <h2 className="text-center mb-4 text-warning">Edit Shop Manager</h2>

      {error && <div className="alert alert-danger">{error}</div>}

      <form onSubmit={handleSubmit} className="shadow p-4 rounded bg-white">
        <div className="mb-3">
          <label className="form-label fw-bold">Shop Manager Name</label>
