...
export default function BlinkitOrderCreate() {
    ...
    useEffect(() => {
        const fetchClients = async () => {
            try {
                const res = await api.get('/blinkitordercreate-data');
                setClients(res.data);
            } catch (error) {
    ...
    const handleItemBlur = () => {
        const match = products.find(item => {
            const combined = `${item.NameBlinkit} (${item.ItemCodeBlinkit})`.toLowerCase().trim();
    ...
    const handleAddItem = () => {
        ...
        setItems([...items, {
            ...
            itemName: `${matchedItem.NameBlinkit} (${matchedItem.ItemCodeBlinkit})`,
    ...
    const handleFileUpload = (e) => {
        ...
        reader.onload = (evt) => {
            ...
            const formattedItems = rawData.map(row => {
                const matchedProduct = products.find(p => p.ItemCodeBlinkit === String(row.Code).trim());
                ...
                return {
                    ...
                    itemName: `${matchedProduct.NameBlinkit} (${matchedProduct.ItemCodeBlinkit})`,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setSuccessMsg('Blinkit order created successfully! 🎉');
            setErrorMsg('');
            setOrderNo('');
            setDate('');
            setPoDate('');
    ...
    return (
        <div className="container mt-5">
            <h2 className="mb-4 text-success fw-bold">Blinkit Order Create</h2>
            ...
            <form onSubmit={handleSubmit} className="shadow p-4 rounded bg-light">
                            ...
                            <tr>
                                <td>
                                    ...
                                    <datalist id="items-options">
                                        {products
                                            .filter(item => item.NameBlinkit && item.ItemCodeBlinkit && item.ItemCodeBlinkit !== "0")
                                            .map(item => (
                                                <option
                                                    key={item.ProductID}
                                                    value={`${item.NameBlinkit} (${item.ItemCodeBlinkit})`}
                                                />
                                            ))
                                        }
                                    </datalist>
                ...
                <button type="submit" className="btn shadow btn-primary w-100" disabled={loading}>
                    ...
                    {loading ? ' Creating...' : 'Blinkit Order Create'}
                </button>
            </form>
