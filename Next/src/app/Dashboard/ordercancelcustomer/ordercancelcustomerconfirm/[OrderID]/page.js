...
export default function OrderCancelCustomerConfirm({ params }) {
    ...
    useEffect(() => {
        ...
        const fetchCustomerOrder = async () => {
            try {
                const res = await api.get(`ordercancelcustomer-show/${OrderID}`);
                ...
                setError("Failed to load customer's order data.");
                console.error(err);
                setLoading(false);
            }
        };
    ...
    const handleDelete = async () => {
        try {
            await api.put(`ordercancelcustomer-cancel/${OrderID}`);
            router.push('/Dashboard/ordercancelcustomer');
        } catch (err) {
            setError("Failed to cancel customer's order.");
    ...
    return (
        <div className="container mt-5">
            <h2 className="text-center mb-4 text-danger fw-bold">Cancel Customer's Order</h2>

            {error && <div className="alert alert-danger">{error}</div>}

            <div className="shadow p-4 rounded bg-white">
                <p className='text-danger fw-bold'>Are you sure you want to cancel the following customer's order?</p>
