...
export default function OrderCancelInstitutionView() {
    ...
    useEffect(() => {
        api.get('ordercancelinstitution-data', {
            params: {
                page: pageIndex + 1,
                limit: pageSize,
                search: globalFilter,
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <button
                    ...
                    onClick={() => router.push(`/Dashboard/ordercancelinstitution/ordercancelinstitutionconfirm/${row.original.OrderID}`)}
                >
                    Cancel
                </button>
            ),
    ...
    return (
        <DataTable
            columns={columns}
            data={data}
            title="Order Cancel Institution"
            pageIndex={pageIndex}
            pageSize={pageSize}
            total={total}
