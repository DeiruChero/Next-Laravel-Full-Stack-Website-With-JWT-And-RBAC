...
export default function PackingMaterialOrderCreate() {
    ...
    useEffect(() => {
        const fetchOrderNo = async () => {
            try {
                const response = await api.get('purchase-invoice-no');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await api.post('packingmaterialpurchase-store', {
            ...
            setSuccessMsg('Packing Material order created successfully! 🎉');
            setErrorMsg('');
            setOrderNo('');
            setDate('');
            setSelectClient('');
    ...
    return (
        <div className="container mt-5">
            <h2 className="mb-4 text-success fw-bold">Create Packing Material Purchase</h2>
