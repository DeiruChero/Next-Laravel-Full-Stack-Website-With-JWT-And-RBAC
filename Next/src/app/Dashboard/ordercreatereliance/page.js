...
export default function RelianceOrderCreate() {
    ...
    useEffect(() => {
        const fetchClients = async () => {
            try {
                const res = await api.get('/relianceordercreate-data');
                setClients(res.data);
            } catch (error) {
    ...
    const handleItemBlur = () => {
        const match = products.find(item => {
            const combined = `${item.NameRelience} (${item.ItemCodeRelience})`.toLowerCase().trim();
    ...
    const handleAddItem = () => {
        ...
        setItems([...items, {
            ...
            itemName: `${matchedItem.NameRelience} (${matchedItem.ItemCodeRelience})`,
    ...
    const handleFileUpload = (e) => {
        ...
        reader.onload = (evt) => {
            ...
            const formattedItems = rawData.map(row => {
                const matchedProduct = products.find(p => p.ItemCodeRelience === String(row.Code).trim());
                ...
                return {
                    ...
                    itemName: `${matchedProduct.NameRelience} (${matchedProduct.ItemCodeRelience})`,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setSuccessMsg('Reliance order created successfully! 🎉');
            setErrorMsg('');
            setOrderNo('');
            setDate('');
            setPoDate('');
    ...
    return (
        <div className="container mt-5">
            <h2 className="mb-4 text-success fw-bold">Reliance Order Create</h2>
            ...
            <form onSubmit={handleSubmit} className="shadow p-4 rounded bg-light">
                            ...
                            <tr>
                                <td>
                                    ...
                                    <datalist id="items-options">
                                        {products
                                            .filter(item => item.NameRelience && item.ItemCodeRelience && item.ItemCodeRelience !== "0")
                                            .map(item => (
                                                <option
                                                    key={item.ProductID}
                                                    value={`${item.NameRelience} (${item.ItemCodeRelience})`}
                                                />
                                            ))
                                        }
                                    </datalist>
                ...
                <button type="submit" className="btn shadow btn-primary w-100" disabled={loading}>
                    ...
                    {loading ? ' Creating...' : 'Reliance Order Create'}
                </button>
            </form>
